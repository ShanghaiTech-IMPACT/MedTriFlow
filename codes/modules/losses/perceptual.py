...
class LPIPS(nn.Module):
    """Learned perceptual distance built from frozen VGG features."""
    ...
    def load_from_pretrained(self, name="vgg_lpips"):
        """Load the bundled linear calibration weights for the selected backbone."""
    ...
    def forward(self, input, target):
        """Return the calibrated perceptual distance between two image batches."""
...
class ScalingLayer(nn.Module):
    """Normalize image channels to the range expected by the perceptual network."""
...
class NetLinLayer(nn.Module):
    """Apply the learned per-channel linear weighting to one feature map."""
...
class vgg16(torch.nn.Module):
    """Expose the VGG16 feature slices used by LPIPS."""
