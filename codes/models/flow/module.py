...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_flash_attention:
            self.attn = FlashAttention(channels, self.num_heads)
        else:
...
class FlashAttention(nn.Module):
    def __init__(self, dim, heads=1):
        ...
        self.to_out = zero_module(nn.Linear(dim, dim, 1))

    def forward(self, x):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)
        
        qkv = self.to_qkv(self.norm(x).permute(0,2,1).contiguous()).chunk(3, dim=2)
        q, k, v = rearrange_many(qkv, 'b d (h c) -> b h d c ', h=self.heads)
        
        out = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=False)
        out = rearrange(out, 'b h d c -> b d (h c)').contiguous()
        
        out = self.to_out(out)
        
        out = (out.permute(0,2,1) + x).reshape(b, c, *spatial)
        return out
